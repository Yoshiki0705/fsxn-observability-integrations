...
| パラメータ | 必須 | 説明 | 例 |
|-----------|------|------|-----|
| `S3AccessPointArn` | ✅ | FSx ONTAP 監査ログ用 S3 Access Point の ARN | `arn:aws:s3:ap-northeast-1:123456789012:accesspoint/fsxn-audit-ap` |
...
> **注意**: `bucket.name` と `object.key` は実際の監査ログパスに合わせて変更してください。`object.key` は FSx ONTAP が出力する監査ログファイルのパスです。
...
LogQL クエリを活用して、FSx ONTAP 監査ログの可視化ダッシュボードを作成します。以下の 4 つのパネルで構成されるダッシュボードにより、ログ量の推移、操作の内訳、ユーザーアクティビティ、失敗イベントを一目で把握できます。
