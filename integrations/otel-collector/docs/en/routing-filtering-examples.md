...
## Routing Connector: By Severity/Operation Type

Route events to different pipelines based on operation type:
...
## Redaction Processor: PII Masking

Mask personally identifiable information before exporting to backends:
...
## Per-Backend Pipeline Separation

Complete configuration with separate pipelines per backend:
...
Suppress high-frequency, low-value events using the filter processor:
...
## Pattern: Delete/Permission Change → Security Backend
...
## Pattern: Read Events → Cheaper Backend or Archive
...
## Pattern: High-Volume Read Logs → Sampled or Dropped
