...
エンタープライズおよびミッションクリティカルワークロード向け:

- **最小**: 1 回の完全な監査ローテーションサイクル（ONTAP 監査ローテーションスケジュールに応じて通常 1-7 日）
- **推奨**: 本番ワークロードでは 2-4 週間
- **ミッションクリティカル**: 正式な運用承認付きで 4 週間以上

並行稼働中の検証項目:
- 新旧パス間のイベント数パリティ（許容範囲: ±1%）
- 新バックエンドですべての重要属性が存在し検索可能
- アラートとランブックが Collector 配信データで正しく機能
- 予期しないレイテンシやデータロスパターンがないこと
