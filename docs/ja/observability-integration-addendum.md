...
|-----------|---------|----------------|-----------|-----------|
| ARP alert | `arw.volume.state` (alert) | T1486 Data Encrypted for Impact | Impact | ✅ 自動封じ込め |
| FPolicy | 大量ファイル削除 (>50/5分) | T1485 Data Destruction | Impact | ✅ 自動封じ込め |
| FPolicy | 大量ファイルリネーム (.encrypted) | T1486 Data Encrypted for Impact | Impact | ✅ 自動封じ込め |
...
**重要な洞察**: SNS トリガートピックが検知と応答の間の「ユニバーサルインターフェース」。JSON を SNS に publish できる任意のシステムが封じ込めをトリガー可能。これがポータビリティレイヤー。
