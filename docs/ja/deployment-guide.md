...
本プロジェクトのスタックは AD 設定の作成・変更を行いません。`automated-response` スタックの SMB ユーザーブロック（name-mapping）は ONTAP SVM レベルで動作し、AD の変更は不要です。ただし:

- ブロック対象ユーザーは `DOMAIN\\username` パターンで識別される
- ドメイン名は ONTAP が使用しているものと一致する必要がある（確認: `vserver cifs show`）
- グループ単位のブロックは未対応（個別ユーザーのみ）
