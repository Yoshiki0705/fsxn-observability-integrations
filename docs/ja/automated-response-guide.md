...
> **ストレージ運用レンズ**: 基盤となる ONTAP メカニズムは同一です。両方のアプローチが同じ REST API エンドポイントを使用します。違いは検知インテリジェンスの配置場所です。専用製品は ML を SaaS に組み込み、本アプローチは利用者が選択した Observability プラットフォームに検知を委ねます。
