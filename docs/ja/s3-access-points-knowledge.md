...
Amazon S3 Access Points for FSx for ONTAP は、FSx ONTAP ボリュームに S3 互換のアクセスポイントをアタッチする機能。NFS/SMB でアクセスしているデータに対して、S3 API 経由でもアクセスできるようになる（データのコピーは不要）。
...
- FSx ONTAP ボリュームが存在し、マウント済み（junction path が設定されている）
- ボリュームが AVAILABLE 状態
...
```json
{
  "S3AccessPointAttachment": {
    ...
    "S3AccessPoint": {
      "ResourceARN": "arn:aws:s3:ap-northeast-1:178625946981:accesspoint/fsxn-audit-observability",
...
s3_client.get_object(
    Bucket="arn:aws:s3:ap-northeast-1:178625946981:accesspoint/fsxn-audit-observability",
...
```json
{
  "Effect": "Allow",
  "Action": ["s3:GetObject"],
  "Resource": ["arn:aws:s3:ap-northeast-1:178625946981:accesspoint/fsxn-audit-observability/object/*"]
}
```
...
| Access Point ARN | `arn:aws:s3:ap-northeast-1:178625946981:accesspoint/fsxn-audit-observability` |
