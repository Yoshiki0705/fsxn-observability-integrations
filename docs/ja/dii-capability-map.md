...
| **Identify（識別）** | 重要な資産・データ・依存関係の棚卸しと理解 | BlueXP<!-- allow:naming --> のデータ分類機能がストレージ全体をスキャンし、データをワークロードの重要度にマッピングして分類 | [データ分類ガイド](../en/data-classification.md) が audit/FPolicy フィールドに対するフィールドレベルの分類マトリクス（PII/Sensitive/Internal）を定義しているが、BlueXP の分類サービスのようにファイル *内容* を自動スキャン/分類するものではない | ⚠️ 組み合わせが必要 — スキーマレベルの分類は存在するが、コンテンツレベルの発見は本当のギャップ |
...
| **Recover（復旧）** | システム・データの復旧、および関係者との復旧連携（CSF 2.0 では RC.RP — Incident Recovery Plan Execution と RC.CO — Incident Recovery Communication に分割） | 検知時点の自動 Snapshot により復旧を簡略化。RC.CO 相当のツールはアラート以上のものは公開されていない | 保護 Snapshot は存在する（Respond フェーズ）が、本リポジトリには**テスト済み・パッケージ化されたリストア Runbook がない** — 下記「本当のギャップ」参照。SNS 通知は最小限の RC.CO 信号をカバーするが、関係者レベルの復旧連携ではない | ⚠️ RC.RP は組み合わせが必要。❌ *検証済みでクリーンな* 復旧ポイントのワークフローはギャップ |

> **レジリエンス成熟度の視点**: CSF 2.0 の RECOVER 機能に関する業界分析（例: [Elastio による ransomware recovery の CSF 2.0 マッピング](https://elastio.com/blog/mapping-ransomware-recovery-to-nist-csf-20)）は、ここで踏まえておくべき重要な指摘をしています — Snapshot やバックアップが存在すること自体は **Protect** のエビデンスであり、RC.RP が運用上信頼できることの証明にはなりません。RC.RP が信頼できると言えるのは、実際にテストされ、侵害されていないことが確認された復旧ポイントを指し示せる場合のみであり、単に Snapshot ジョブが完了したことではありません。本リポジトリの自動応答モジュールは保護 Snapshot を作成します（Respond フェーズ）が、それがクリーンであることの検証や、そこからのリストアの試験実施はまだ行っていません。「Snapshot が作成された」ことと「検証済みでクリーンな復旧ポイントがあり、テスト済みである」ことは異なる成熟度レベルとして扱ってください。本リポジトリが現在提供しているのは前者です。
...
3. **パッケージ化された Snapshot リストア Runbook がなく、検証済みでクリーンな復旧ポイントのワークフローもない。** Respond フェーズは保護 Snapshot を作成しますが、Recover フェーズでの実際のリストアは、本リポジトリではまだスクリプト/ガイドに落とし込まれていない手動 ONTAP 操作です。より重要な点として（上記の CSF 2.0 RECOVER の議論の通り）、本リポジトリは、あるSnapshot をリストア候補として扱う前に、それが侵害されていないことを検証していません — Snapshot が存在することと、テスト済みでクリーンと確認された復旧ポイントであることは同じではありません。
...
5. **Govern 機能のツールとコンテンツレベルのデータ分類/発見がない。** リスク管理戦略、ポリシー、取締役会レベルの報告（CSF 2.0 の Govern）は組織側の責任であり、本リポジトリはこれを自動化しようとしていません。同様に、本リポジトリの [データ分類ガイド](../en/data-classification.md) はスキーマレベルの分類（どの *フィールド* が PII か）を定義していますが、CSF 2.0 の Identify 機能に対して BlueXP<!-- allow:naming --> のデータ分類サービスが行うようなファイル *内容* のスキャンは行っていません。
...
- [データ分類ガイド](../en/data-classification.md) — Forensics ダッシュボードで表示される user/IP/path フィールドの PII 取り扱い、および上記 Identify 機能のスキーマ分類
