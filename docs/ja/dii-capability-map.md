...
| Recover | 検知時点 Snapshot からの高速リストア | Respond フェーズで作成した保護 Snapshot からの手動 `volume snapshot restore` | ⚠️ 組み合わせが必要 — リストア自体は依然手動の ONTAP 操作 | 下記「本当のギャップ」参照 |
| Recover | リストア前の検証済みクリーン復旧ポイント（RC.RP） | FlexClone + 隔離された S3 Access Point スキャン + 判定結果の記録 — [検証済みクリーン復旧ポイントガイド](verified-recovery-point-guide.md) 参照 | ✅ 完全対応（高速な事前フィルタであり、完全なフォレンジックスキャンではない） | 本ドキュメント、「本当のギャップ」項目3（解決済み） |
| Identify | コンテンツレベルの PII/データ分類 | Amazon Comprehend `DetectPiiEntities` を S3 Access Point 経由で実行 — [コンテンツレベル PII 分類スキャナー](content-classification-scanner.md) 参照 | ✅ テキスト/構造化データ形式は完全対応。❌ Office/PDF のコンテンツ抽出はギャップ | 本ドキュメント、「本当のギャップ」項目5（テキスト形式は解決済み） |
...
3. ~~パッケージ化された Snapshot リストア Runbook がなく、検証済みでクリーンな復旧ポイントのワークフローもない。~~ **解決済み。** [検証済みクリーン復旧ポイントガイド](verified-recovery-point-guide.md) が、検証対象 Snapshot を FlexClone として複製し、隔離された S3 Access Point 経由でランサムウェア関連のファイル拡張子をスキャンし、合否判定を記録します — 人間がリストアを決断する前に実行されます。これは依然として高速で粗い事前フィルタ（拡張子パターンマッチング）であり、完全なフォレンジックグレードのコンテンツスキャンや実際のエンドツーエンドのリストアリハーサルではありません。「検証済み」がここで何を意味するかの正確な境界は、同ガイドの比較セクションを参照してください。
...
5. ~~コンテンツレベルのデータ分類/発見がない。~~ **テキスト/構造化データ形式については解決済み。** [コンテンツレベル PII 分類スキャナー](content-classification-scanner.md) が Amazon Comprehend `DetectPiiEntities` によりファイル内容をスキャンし、本リポジトリの [データ分類ガイド](../en/data-classification.md) のスキーマレベル（フィールド名）分類を補完します。本当に残っている限界: ドキュメント形式のパースは未対応で、Office ドキュメントや PDF はテキスト抽出されないため、これらの形式のコンテンツレベル PII は検出されません（詳細は同ガイドの「本当の限界」参照）。
