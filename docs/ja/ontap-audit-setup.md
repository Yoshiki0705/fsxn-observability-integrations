...
> **参考**
>
> [AWS ドキュメント — ファイルアクセス監査](https://docs.aws.amazon.com/fsx/latest/ONTAPGuide/file-access-auditing.html)
...
> **注意**
>
> 監査を有効化する前にジャンクションパスが存在している必要があります。ONTAP はこのパスに監査ログを書き込みます。
...
> **本プロジェクトでの注意**
>
> `shared/lambda-layers/log-parser/` は EVTX と XML の両フォーマットに対応しています。運用上の好みに応じて選択してください。
...
> **設計ルール**
>
> 本プロジェクトの Lambda ログプロセッサは、ローテーション済みファイル（`audit_*.evtx` または `audit_*.xml` パターンに一致するファイル）のみを読み取ります。アクティブログファイルは常にスキップされます。
...
> **推奨**
>
> ボリュームの自動拡張とローテーション制限を組み合わせてください。ボリューム使用率を監視し、80% でアラートを設定してください。
...
> **トラブルシューティング**
>
> S3 Access Point 経由でファイルが表示されない場合、以下を確認してください：
