...
> **Retry-policy note**: Both the [Verified-Clean Recovery Point Guide](verified-recovery-point-guide.md)'s Step Functions state machine and the notification path in the [Content-Level PII Classification Scanner](content-classification-scanner.md) share a pattern worth naming once at this level: neither implements automatic retry for transient errors — the recovery guide's states define `Catch` blocks with no `Retry` blocks, and both stacks' SNS `publish` calls fail silently into a log line rather than surfacing as an alarm-able error. A transient throttle or network blip in either pipeline is either treated as a permanent failure (recovery guide) or produces a notification that quietly never arrives (both). If you build monitoring against either capability, alarm on the underlying CloudWatch Logs patterns each guide documents in its own notes, rather than assuming Step Functions execution status or Lambda invocation success alone tells the whole story.

> **Encryption-at-rest note**: The DynamoDB tables in both the [Verified-Clean Recovery Point Guide](verified-recovery-point-guide.md) and [Content-Level PII Classification Scanner](content-classification-scanner.md) enable encryption (`SSEEnabled: true`) but use the AWS-owned DynamoDB key rather than a customer-managed KMS key in both templates, and neither template exposes a parameter to change this. This is a reasonable default and satisfies "encrypted at rest" in most compliance frameworks, but if your organization's key-management policy specifically requires customer-managed KMS keys (e.g., for independently auditable key rotation or a cross-account key-sharing model), both tables — and both stacks' CloudWatch Log Groups — would need template changes to add `SSEType: KMS`/`KMSMasterKeyId` before deployment.
...
A: Not necessarily — both capabilities swallow SNS publish failures into a log line rather than an alarm, per the SNS-delivery notes in each guide (see the Retry-policy note above for the cross-cutting pattern). Check the underlying DynamoDB record directly (`verdict`/`files_with_pii`) rather than treating "no notification arrived" as equivalent to "nothing was found." This is a gap worth closing with your own CloudWatch Alarm if either notification path is part of an incident-response trigger chain.
