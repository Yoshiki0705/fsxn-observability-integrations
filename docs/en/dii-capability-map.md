...
5. **Content-level PII discovery covers text/structured-data formats only.** The [Content-Level PII Classification Scanner](content-classification-scanner.md) closes the original "no content-level classification" gap for `.txt`/`.csv`/`.json`/`.log` and similar formats via Amazon Comprehend `DetectPiiEntities`, complementing this repo's [Data Classification Guide](data-classification.md) schema-level (field-name) classification. What remains genuinely unaddressed: no document-format parsing — Office documents and PDFs are not text-extracted, so content-level PII in those formats is not detected (see that guide's Genuine Limitations section).
