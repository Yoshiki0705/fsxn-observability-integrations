...
| Recover | Fast restore from detection-time snapshot | Manual `volume snapshot restore` from the protective snapshot created during Respond | ⚠️ Requires assembly — restore itself remains a manual ONTAP operation | See Gaps below |
| Recover | Verified-clean recovery point (RC.RP) before restore | FlexClone + isolated S3 Access Point scan + recorded verdict — see [Verified-Clean Recovery Point Guide](verified-recovery-point-guide.md) | ✅ Full (fast pre-filter, not a full forensic scan) | This document, Genuine Gaps item 3 (resolved) |
| Identify | Content-level PII/data classification | Amazon Comprehend `DetectPiiEntities` via S3 Access Point — see [Content-Level PII Classification Scanner](content-classification-scanner.md) | ✅ Full for text/structured-data formats; ❌ Gap for Office/PDF content extraction | This document, Genuine Gaps item 5 (resolved for text formats) |
...
3. ~~No packaged snapshot-restore runbook, and no verified-clean recovery point workflow.~~ **Resolved.** The [Verified-Clean Recovery Point Guide](verified-recovery-point-guide.md) clones the candidate snapshot (FlexClone), scans it via an isolated S3 Access Point for ransomware-associated file extensions, and records a pass/fail verdict — before a human commits to a restore. This remains a fast, coarse pre-filter (extension-pattern matching), not a full forensic-grade content scan or an actual end-to-end restore rehearsal; see that guide's Comparison section for the precise boundary of what "verified" means here.
...
5. ~~No content-level data classification/discovery.~~ **Resolved for text/structured-data formats.** The [Content-Level PII Classification Scanner](content-classification-scanner.md) scans file contents via Amazon Comprehend `DetectPiiEntities`, complementing this repo's [Data Classification Guide](data-classification.md) schema-level (field-name) classification. Genuine remaining limitation: no document-format parsing — Office documents and PDFs are not text-extracted, so content-level PII in those formats is not detected (see that guide's Genuine Limitations section).
...
- [Verified-Clean Recovery Point Guide](verified-recovery-point-guide.md) — RC.RP verification (FlexClone + isolated scan + recorded verdict), closing Genuine Gaps item 3
- [Content-Level PII Classification Scanner](content-classification-scanner.md) — Content-level PII discovery via Amazon Comprehend, closing Genuine Gaps item 5 for text/structured-data formats
