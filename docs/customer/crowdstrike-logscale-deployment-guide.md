...
## パフォーマンス比較: ALog Converter vs 本ソリューション

| 比較項目 | ALog Converter | 本ソリューション (LogScale) |
|---------|---------------|--------------------------|
| **ログ収集間隔** | 15〜60分 (バッチ) | **1〜5分** (EventBridge Scheduler) |
| **リアルタイムパス** | なし | **サブ秒** (FPolicy → ECS → SQS → Lambda) |
| **パース速度** | 非公開 (体感: 数分/GB) | **178,000 events/sec** (実測) |
| **検索応答** | 数秒〜数十秒 (DB依存) | **サブ秒** (LogScale index-free) |
| **スケーラビリティ** | サーバー固定 (垂直スケール) | **Lambda 自動スケール** (水平) |
| **運用負荷** | サーバー管理、パッチ適用 | **ゼロ運用** (サーバーレス) |
| **保持期間** | DB 容量依存 | ライセンス次第 (7日〜無制限) |
| **コスト** | ライセンス + サーバー費 | **~$1/月** (AWS 側) + LogScale ライセンス |
...
→ ALog Converter の一般的な処理時間 (数分〜数十分) に対して 100〜1000倍高速
```

### ALog Converter からの移行メリット

1. **レイテンシ改善**: 15-60分 → 1-5分 (最大60倍改善)
...
5. **コスト削減**: ALog ライセンス + サーバー費 vs Lambda ~$1/月
...
ONTAP 監査ログは Windows Security Event Log と同じ EventID 体系を使用しています。ALog Converter で慣れた EventID がそのまま使えます:

| EventID | 意味 | ALog 相当 | LogScale クエリ |
|---------|------|----------|----------------|
| 4663 | ファイルアクセス | ファイル参照 | `event_type="4663"` |
| 4656 | ハンドル要求 | ファイルオープン | `event_type="4656"` |
| 4660 | ファイル削除 | ファイル削除 | `event_type="4660"` |
| 4658 | ハンドルクローズ | ファイルクローズ | `event_type="4658"` |

> ALog で使用していた EventID ベースのフィルタリングルールは、そのまま LogScale のクエリに移植できます。
