...
aws secretsmanager create-secret \
  ...
  --description "FSx ONTAP admin credentials for Management Console" \
  --secret-string '{"username": "fsxadmin", "password": "<your-password>"}'
```

### S3 Access Point（ファイルブラウザ機能を使用する場合）

FSx ONTAP S3 Access Point が作成済みであること。ARN 形式:

```
arn:aws:s3:<region>:<account-id>:accesspoint/<access-point-name>
```

### FSx ONTAP セキュリティグループ

FSx ONTAP ファイルシステムのセキュリティグループ ID が必要です。Harvest と ToolJet がポート 443 で管理エンドポイントにアクセスするため、デプロイスクリプトがこの SG にルールを自動追加します。

```bash
# FSx ONTAP のセキュリティグループ ID を確認
...
| `OntapManagementEndpoint` | `String` | ✅ | — | FSx ONTAP 管理エンドポイント IP/DNS |
...
| `S3AccessPointArn` | `String` | ✅ | — | FSx ONTAP S3 Access Point ARN |
...
4. FSx ONTAP メトリクスがダッシュボードに反映されていることを確認（デプロイ後 2〜3 分）
...
- FSx ONTAP のセキュリティグループで ECS タスクからのアクセスが許可されていることを確認
...
**原因**: FSx ONTAP のセキュリティグループに Harvest/ToolJet タスク SG への参照が残っている。
